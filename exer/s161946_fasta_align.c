...
float	sw_alignment( float **m,		/* Scoring matrix, d matrix from lecture */
...

{
	...
	/* Remember
             ...
             S[m][n] = max{   P[m][n]
                              ...
                              S[m][n+1] - fg
             P[m][n] = max{   P[m][n+1] - ng


                              S[m+1][n] - fg
             Q[m][n] = max{   Q[m+1][n] - ng
...
/* Fill in the missing code XXXXX */
			...
			temp1 = Q[i + 1][j] - ng; /* Gap extension */
            		temp2 = S[i + 1][j] - fg; /* Gap opening   */
			...
			temp1 = P[i][j + 1] - ng; /* Gap extension */
            		temp2 = S[i][j + 1] - fg; /* Gap opening   */
...
/* This is some code you might use to debug the program */
	...
	/* Due to underflow we do not test for a difference to be exactly 0, but
	rather less than the value SMALL */
